...
class photo_uploader extends vengine_mod
{
	...
	public function flush_files()		// READY
	{
		foreach($this->sess_files as $file)
		{
			if($file['tmp_path']) unlink($file['path']);
		}
		$this->sess_files=array();
	}
	public function __construct($args)		// READY
	{
			$this->init($args);
			if(!isset($this->CNF['MAX_FILES'])) $this->CNF['MAX_FILES']=0; // unlimited, no mandatory configuration options
			if(!isset($this->CNF['INSTANCE_NAME'])) $this->CNF['INSTANCE_NAME']='V_UPLOADER';
			if(!isset($_SESSION[$this->CNF['INSTANCE_NAME']]['V_FILE_UPLOADS'])) $_SESSION[$this->CNF['INSTANCE_NAME']]['V_FILE_UPLOADS']=array();
			$this->sess_files=&$_SESSION[$this->CNF['INSTANCE_NAME']]['V_FILE_UPLOADS']=array();
			if(!isset($_SESSION[$this->CNF['INSTANCE_NAME']]['V_LAST_UPLOADED_FILE'])) $_SESSION[$this->CNF['INSTANCE_NAME']]['V_LAST_UPLOADED_FILE']='';
			$this->last_uploaded_status=&$_SESSION[$this->CNF['INSTANCE_NAME']]['V_LAST_UPLOADED_FILE'];
			$this->files_dir=$this->CNF['FILES_DIR'].'/'.$this->USR->get_uid().'/';
			if(!is_dir($this->files_dir))
			{
			 	if(!mkdir($this->files_dir))
			 	{
			 		 $this->msg('CANNOT_MKDIR');
			 		 return;
			 	}
			}
	}
